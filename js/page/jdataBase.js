...
function createHideLinkForMannul()
{
    var beginNode = null;
        ...
        //节点连线
        g_CurSence.GetObjId().mouseup(function(e){
            if((tmpLineAllow == false) && (tmpPolyLineAllow == false))
                return;
            if(e.button == 2){
                g_CurSence.GetObjId().remove(link);
                beginNode = null;
                return;
            }
        ...
        g_CurSence.GetObjId().mousedown(function(e){
            if((tmpLineAllow == false) && (tmpPolyLineAllow == false))
                return;
        ...
        g_CurSence.GetObjId().mousemove(function(e){
            if((tmpLineAllow == false) && (tmpPolyLineAllow == false))
                return;
            tempNodeZ.setLocation(e.x, e.y);
        });
