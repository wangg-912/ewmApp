...
function creatIframe(href,titleName){
	...
	iframeBox.hide();
	if(titleName == "拓扑编排")
        iframe_box.append('<div class="show_iframe" style="z-index: 0" id="拓扑编排"><div class="loading"></div><iframe frameborder="0" src='+href+'></iframe></div>');
	else
...
$(function(){
	...
	$(document).on("click","#min_title_list li",function(){
		...
		var tmpLi = $(this).find('span').attr("data-href");
		$("#min_title_list li").removeClass("active").eq(bStopIndex).addClass("active");

		if(tmpLi != "TopoMng/jTopo_link.html") {
            //iframe_box.find(".show_iframe").hide().eq($(this).attr('id')).show();
            iframe_box.find(".show_iframe").not("#拓扑编排").hide();
            iframe_box.find(".show_iframe").eq(bStopIndex).show();
        }
		else
		{
			iframe_box.find(".show_iframe").not("#拓扑编排").hide();
